...
export type TreeRow = {
  ...
  selection: Selection;
  expandable: boolean;
  expanded?: boolean;  // current state, derived from the store's expansion map
};
...
export function buildTreeRows(
  ...
  for (const b of parsed.blocks) {
    const blockId = `block:${b.index}`;
    const blockExpanded = isExpanded(blockId, expansion);
    rows.push({
      depth: 0,
      id: blockId,
    ...
    if (b.body.kind === 'huffman' && b.body.dynamicMeta) {
      ...
      rows.push({
        ...
        detail: `litlen ${b.body.litlenTable.symbolsByCode.length} · dist ${b.body.distTable.symbolsByCode.length}`,
        selection: { kind: 'blockSection', blockIndex: b.index, section: 'huffman-tables' },
        expandable: false,
      });
    }

    if (b.body.kind === 'huffman') {
      const symbolsId = `symbols:${b.index}`;
      const symbolsExpanded = isExpanded(symbolsId, expansion);
      rows.push({
        depth: 1,
        id: symbolsId,
      ...
      if (symbolsExpanded && depth >= 3) {
        for (let i = 0; i < b.body.symbols.length; i++) {
          ...
          rows.push({
            depth: 2,
            label,
            rangeText: rangeText(rng),
            selection: { kind: 'symbol', blockIndex: b.index, symbolIndex: i },
            expandable: false,
          });
        }
      }
